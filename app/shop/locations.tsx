...
function Locations() {
    ...
    const LocationCard = ({ data, handleSelect }: any) => {
        return (
            <TouchableOpacity onPress={() => handleSelect({ ...data, area: area })} className="flex-row gap-4 items-center justify-between bg-white border-b shadow-sm border-gray-200 p-4 rounded-xl">
            ...
            <View className="p-4">
                ...
                <View className="flex-wrap flex-row gap-[9px]">
                    {topSearches.map((item: any, index: number) => {
                        return (
                            <TouchableOpacity onPress={() => setArea(item)} className={`flex-row px-4 py-2 rounded-2xl self-start border border-slate-300 ${item === selectedArea ? 'bg-slate-600' : 'bg-[#e4f0ff]'}`} key={index}>
                                <Text className={`font-PoppinsMedium text-[12px] ${item === selectedArea ? 'text-white' : 'text-sky-700'}`}>{item}</Text>
                            </TouchableOpacity>
                        )
                    })}
                </View>
