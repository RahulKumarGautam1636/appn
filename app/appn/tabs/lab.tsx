...
import { addToCart, setAppnData, setModal } from '@/src/store/slices/slices';
...
const LabTests = ({}: any) => {
    ...
    const [testItems, setTestItems] = useState({ loading: true, data: { ItemSupplierLinkList: [] }, err: { status: false, msg: "" } });
    const [invDepartments, setInvDepartments] = useState({ loading: true, data: { LinkSubCategoryList: [] }, err: { status: false, msg: "" } });
    const [selectedDept, setSelectedDept] = useState({});
    // const filteredDepartments = invDepartments.data.LinkSubCategoryList.filter((i: any) => ((i.CategoryDesc).toLowerCase()).includes((filterDept).toLowerCase()));
    const [selectionActive, setSelectionActive] = useState(false);
    const allDepartments = useSelector((i: RootState) => i.menu.departments);
    const department = allDepartments.find((i: any) => i.DeptCategory.includes('INVESTIGATION'));

    const cart = useSelector((i: RootState) => i.cart)
    const cartArray = Object.values(cart);
    const labTests = cartArray.filter((i: any) => i.type === 'LABTEST');

    useEffect(() => {
        const controller = new AbortController();
        const getLabData = async (company: any, deptId: any, categoryId: any, signal: AbortSignal) => {
            if (!company.EncCompanyId) return console.log('no companyCode received');
            if (!categoryId) return console.log('no Loc Id received');
            const res = await getFrom(`${BASE_URL}/api/item/Get_AllLinkItems?CID=${company.EncCompanyId}&ItemGroupId=0&ItemId=0&DeptId=${deptId}&SubCatId=${categoryId}&TPAId=0&PartyCode=0`, {}, setTestItems, signal);
            if (res) {
                const tests = (res.data.ItemSupplierLinkList.slice(0, 50)).map((i: any) => ({ 
                    Description: i.Description, 
                    SRate: i.Rate, 
                    // OrgRate: i.OrgRate, 
                    DiscountPer: i.DiscPer,
                    ItemId: i.ItemId, 
                    LocationItemId: i.ItemId, 
                    AutoId: i.AutoId, 
                    DepartmentId: i.CostSubcode, 
                    SubCategoryId: i.SubCategoryId,
                    CategoryName: i.ItemGroup,
                    IGSTRATE: i.IGSTRATE,
                    count: 1
                }));
                setTestItems(pre => ({ ...pre, loading: false, data: { ...res.data, ItemSupplierLinkList: tests } }));
            }
        }
        getLabData(selectedCompany, department?.DeptId, selectedDept.CategoryId, controller.signal);
        setSearchItem({ name: '' });
        return () => controller.abort();
    }, [selectedCompany.EncCompanyId, department?.DeptId, selectedDept.CategoryId]);

    useEffect(() => {
        let controller = new AbortController();
        const getInvDepartments = async (companyCode: string, parentId: string) => {
        if (!parentId) return;
        const res = await getFrom(`${BASE_URL}/api/Pharma/GetCatSubCat?CID=${companyCode}&LOCID=0&CategoryId=0&ParentId=${parentId}`, {}, setInvDepartments, controller.signal);
            if (res) {
                setTimeout(() => {
                    setInvDepartments(res);
                    setSelectedDept((res.data.LinkSubCategoryList[1]))
                }, 500);
            }
        };
        getInvDepartments(selectedCompany.EncCompanyId, department?.DeptId);
        return () => controller.abort();
    }, [selectedCompany.EncCompanyId, department?.DeptId]);

    useEffect(() => {
        dispatch(setAppnData({ labTestDate: new Date(date.value).toLocaleDateString('en-TT') }))
    }, [date.value])

    const handleDate = () => {
        if (labTests.length) return;
        setDate(pre => ({...pre, active: true}))
    }    
        ...
        <ScrollView contentContainerClassName='bg-slate-100 min-h-full'>
            ...
            <View className=''>
                ...
                <View className='p-3'>
                    ...
                    <RenderLabTest data={testItems} dateValue={date.value} />
                </View>
            </View>
            </GradientBG>
        </ScrollView>
...
const RenderLabTest = memo(({ data, dateValue }: any) => {
    ...
    // const renderSlider = (data: any) => {    
        if (data.loading) {
          ...
          return;
        } else if (data.data.ItemSupplierLinkList.length === 0) {
          return;
        } else {
            return (    // <Text className='font-PoppinsMedium my-3'>{parentCategoryName}</Text>
                <View className='gap-3'>        
                    {data.data.ItemSupplierLinkList.map((i: any) => <LabCard key={i.LocationItemId} data={i} testDate={(dateValue).toLocaleDateString('en-TT')} />)}    
                </View>
            ) 
        }
    // }
