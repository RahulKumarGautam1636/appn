    ...
    return (
        <>
            <ScrollView contentContainerStyle={styles.screen} contentContainerClassName='bg-slate-100'>
                ...
                <View className='px-2 pb-1 flex-row justify-around'>
                    {(() => {
                        if (dateTabsList.loading) {
                            ...
                            return dateTabsList.data.slice(0, 5).map((i: any) => <DayBtn key={i.SDateStr} date={i.Day} day={i.DName.substring(0, 3)} active={i.SDateStr === selectedDate} handleActive={() => handleDateChange(i)} />)
                        }
                    })()}
                </View>
                ...
                <ButtonPrimary title='Book Appointment' active={true} onPress={toggleConfirmation} classes='m-4' />
            </ScrollView>
            <Animated.View 
